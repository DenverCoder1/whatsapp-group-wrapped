...
function runTest(testName, inputFile, expectedOutput) {
    ...
    try {
        ...
        const command = `node script/main.js "${inputFile}"`;
...
function createZipFile() {
    ...
    try {
        ...
        try {
            const pythonScript = `import zipfile
import os

source_dir = ${JSON.stringify(testDir)}
zip_path = ${JSON.stringify(zipFile)}
...
`;
            const tempScript = path.join(__dirname, ".temp_zip_script.py");
...
function main() {
    ...
    const txtFile = path.join(
        ...
        "WhatsApp Chat with Pomegranate Community.txt"
    );
